...
template <typename DT>
Matrix <DT> solveCholesky (Matrix <DT> A, Matrix <DT> b) {
    if (A.M != A.N) {
        Matrix <DT> ret;
        ret._errMessage = 1;
        return ret;
    ...
    for (int k = 1; k <= A.M; ++ k) {
        A.arr [k][k] = sqrt (A.arr [k][k]);
        for (int i = k + 1; i <= A.M; ++ i)
            A.arr [i][k] /= A.arr [k][k];
        for (int j = k + 1; j <= A.M; ++ j)
            for (int i = j; i <= A.M; ++ i)
                A.arr [i][j] -= A.arr [i][k] * A.arr [j][k];
    }
...
template <typename DT>
Matrix <DT> solveImprovedCholesky (Matrix <DT> A, Matrix <DT> b) {
    if (A.M != A.N) {
        Matrix <DT> ret;
        ret._errMessage = 1;
        return ret;
    ...
    for (int j = 1; j <= A.M; ++ j) {
        for (int i = 1; i < j; ++ i)
            v [i] = A.arr [j][i] * A.arr [i][i];
        for (int i = 1; i < j; ++ i)
            A.arr [j][j] -= A.arr [j][i] * v [i];
        for (int i = j + 1; i <= A.M; ++ i)
            A.arr [i][j] /= A.arr [j][j];
        for (int i = j + 1; i <= A.M; ++ i)
            for (int k = 1; k < j; ++ k)
                A.arr [i][j] -= A.arr [i][k] * v [k] / A.arr [j][j];
    }
